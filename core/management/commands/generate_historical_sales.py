"""
Generate realistic historical sales data for the past 90-100 days
with seasonal patterns, price variations, and stock movements
"""
...
class Command(BaseCommand):
    help = 'Generate 90-100 days of realistic historical sales data'
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=95,
            help='Number of days of historical data to generate (default: 95)',
        )
    ...
    def handle(self, *args, **options):
        ...
        # Date range: from (today - days) to today
        ...
        # Reset stock to 0 before starting
        ...
        # Restore original values but leave 1-5 stock for realism
        self.stdout.write('Restoring original product values with remaining stock...')
        for product in products:
            orig = original_values[product.product_id]
            # Leave 1-5 stock remaining instead of original stock
            remaining_stock = random.randint(1, 5)
            is_kg = (product.quantity_unit or '').lower() == 'kg'
            if is_kg:
                remaining_stock = round(random.uniform(1.0, 5.0), 2)
            
            product.stock = remaining_stock
            product.price = orig['price']
            product.cost = orig['cost']
            product.save(update_fields=['stock', 'price', 'cost'])
        
        self.stdout.write(self.style.SUCCESS(
            f'[OK] Generated {total_sales} sales and {total_stock_additions} stock additions over {days} days'
        ))
    ...
    def _add_stock_to_product(self, date, product):
        ...
        if is_kg:
            quantity = round(random.uniform(20, 60), 2)
        else:
        ...
        days_ago = (timezone.now().date() - date.date()).days
        cost_variance = 1.0 - (random.uniform(0.05, 0.15) * (days_ago / 100))
        stock_cost = max(5.0, round_to_5(base_cost * cost_variance))
        
        price_variance = 1.0 - (random.uniform(0.03, 0.10) * (days_ago / 100))
    ...
    def _generate_sales_for_product(self, date, product, user, num_items=1, transaction_number=None):
        ...
        # Use varied historical price (rounded to 5)
        current_price = float(product.price)
        days_ago = (timezone.now().date() - date.date()).days
        price_variance = 1.0 - (random.uniform(0.03, 0.12) * (days_ago / 100))
